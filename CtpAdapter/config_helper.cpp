...
const string ConfigHelper::kConfigPath{ "/config/ctp.json" };

// -----------------------------------------------------------------------------------------------;
// json functions
// -----------------------------------------------------------------------------------------------;
void from_json(const json& j, CtpAccount& cfg) {
...
void from_json(const json& j, CtpConfig& cfg) {
...
bool ConfigHelper::Load() {
    try {
        ...
        Logger::Log()->error("[{}:{}] Failed to init CTP config. {}", __func__, __LINE__, e.what());
        return false;
    }
}
