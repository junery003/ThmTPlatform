...
void TraderClient::Connect() {
    ...
    Logger::Log()->info("[{}:{}] CTP TR Initial client: serverUri={}, broker={}, user={}", __func__, __LINE__,
...
void TraderClient::OnFrontConnected() {
    Logger::Log()->info("[{}:{}] CTP TR Front connected.", __func__, __LINE__);
...
void TraderClient::OnFrontDisconnected(int nReason) {
    ...
    Logger::Log()->error("[{}:{}] CTP TR Front disconnected: reasonCode={}", __func__, __LINE__, nReason);
...
void TraderClient::OnRspUserLogin(CThostFtdcRspUserLoginField* pRspUserLogin,
    ...
    if (pRspInfo != nullptr && pRspInfo->ErrorID != 0) {
        Logger::Log()->error("[{}:{}] CTP TR Failed to login, errorID={}, errorMsg={}",
    ...
    if (pRspUserLogin == nullptr) { // Should not happen
        Logger::Log()->error("[{}:{}] CTP TR invalide server response, got null pointer of login info", __func__, __LINE__);
        logon_state_ = LogonState::LOGON_FAILED;
        return;
    }
    ...
    Logger::Log()->info("[{}:{}] CTP TR Login successfully: broker={}, user={}, sessionId={}, tradingDay={}", __func__, __LINE__,
...
void TraderClient::OnRspAuthenticate(CThostFtdcRspAuthenticateField* pRspAuthenticateField,
    CThostFtdcRspInfoField* pRspInfo, int nRequestID, bool bIsLast) {
    if (pRspInfo != nullptr && pRspInfo->ErrorID != 0) {
        Logger::Log()->error("[{}:{}] CTP TR Failed to authenticate, errorID={}, errorMsg={}", __func__, __LINE__,
            pRspInfo->ErrorID, pRspInfo->ErrorMsg);
        return;
    }

    if (pRspAuthenticateField == nullptr) {
        Logger::Log()->error("[{}:{}] CTP TR Invalid server response, got null pointer of authentication info.", __func__, __LINE__);
        return;
    }

    Logger::Log()->info("[{}:{}] CTP TR Authenticate succeed: brokerID={}, userID={}, appID={}, appType={}.",
...
void TraderClient::QueryInstrument(const char* symbol, const char* exchangeID) {
    EnsureLogon();
...
void TraderClient::EnsureLogon() {
    ...
    if (logon_state_ != LogonState::LOGON_SUCCEED) {
        Logger::Log()->error("[{}:{}] CTP TR logon failed.", __func__, __LINE__);
    }
}
...
void TraderClient::DoLogin() {
    ...
    if (rtnCode != 0) {
        Logger::Log()->info("[{}:{}] CTP TR Login Request failed: code={}", __func__, __LINE__, rtnCode);

        //this_thread::sleep_for(3s);
        //doLogin();
    }
    else {
        Logger::Log()->info("[{}:{}] CTP TR Login request successfully.", __func__, __LINE__);
    }
}
...
void TraderClient::DoAuthenticate() {
    ...
    if (rtnCode != 0) {
        Logger::Log()->error("[{}:{}] CTP TR Authenticate Request failed: code=[{}].", __func__, __LINE__, rtnCode);
    }
    else {
