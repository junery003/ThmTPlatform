...
void MarketDataClient::Connect() {
    string tmpFolder("atp-md/");
    if (!filesystem::exists(tmpFolder)) {
        filesystem::create_directory(tmpFolder);
    }

    md_api_ = CThostFtdcMdApi::CreateFtdcMdApi(tmpFolder.c_str()); // "atp-md/"
    ...
    Logger::Log()->info("[{}:{}] ATP MD Initial client: serverUri={}, broker={}, user={}", __func__, __LINE__,
...
void MarketDataClient::OnFrontConnected() {
    Logger::Log()->info("[{}:{}] ATP MD Front connected.", __func__, __LINE__);
...
void MarketDataClient::DoLogin() {
    ...
    if (rtnCode != 0) {
        Logger::Log()->error("[{}:{}] ATP MD Login Request failed: code={}", __func__, __LINE__, rtnCode);
        //this_thread::sleep_for(5s);
        //doLogin();
    }
    else {
        Logger::Log()->info("[{}:{}] ATP MD Login request successfully.", __func__, __LINE__);
    }
}

void MarketDataClient::OnFrontDisconnected(int nReason) {
    Logger::Log()->error("[{}:{}] ATP MD Front disconnected: reasonCode={}", __func__, __LINE__, nReason);
...
void MarketDataClient::OnRspUserLogin(CThostFtdcRspUserLoginField* pRspUserLogin,
    ...
    if (pRspInfo == nullptr) {
        Logger::Log()->error("[{}:{}] ATP MD Logon failed with no reason.", __func__, __LINE__);
        return;
    }
    ...
    Logger::Log()->info("[{}:{}] ATP MD Login successfully: broker={}, user={}, sessionId={}, tradingDay={}", __func__, __LINE__,
