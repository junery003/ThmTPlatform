...
namespace ThmAtpIntegrator.AtpHandler {
    public class AtpTradeHandler : TradeHandlerBase {
        private static readonly NLog.ILogger Logger = NLog.LogManager.GetCurrentClassLogger();

        private int _position = 0;
        public AtpTradeHandler(AtpInstrumentHandler instrumentHandler) : base(instrumentHandler) {
        }
        ...
        internal OrderData BuildOrderData(AtpOrderData orderMsg) {
            ...
            if (string.IsNullOrWhiteSpace(orderMsg.OrderID) || orderMsg.OrderStatus.StartsWith(EOrderStatus.Failed.ToString())) {
                orderData = new OrderData(id) {
                    Provider = orderMsg.Provider,  // "ATP"
